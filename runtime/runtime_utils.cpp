...
void generate_sdesc_yaml_for_overlay_compile(std::set<chip_id_t> chips, tt::ARCH arch_name, bool noc_trans_en){
    ...
    fout << device_descs;
    fout.close();
    // If noc translation is enabled during compile or run, the runtime descs are in device_desc_runtime.
    // If not, runtime and pipegen soc descriptors are shared in device_descs.
    std::string n2p_soc_desc_dir = noc_trans_en ? "/device_desc_runtime/" : "/device_descs/";
    if(arch_name != tt::ARCH::GRAYSKULL and fs::exists(tt::io::info.output_dir + n2p_soc_desc_dir)) {
        // Give Net2Pipe access to per chip soc descriptors if harvesting was performed (runtime device descs exists)
        // and if workload is running on non-Grayskull
        YAML::Node device_descs_net2pipe;
        for(auto it = chips.begin(); it != chips.end(); it++)
            device_descs_net2pipe["chip_descriptors"][std::to_string(*it)] = tt::io::info.output_dir + n2p_soc_desc_dir + std::to_string(*it) + ".yaml";
        std::ofstream fout_n2p(tt::io::info.output_dir + "/device_descs_for_net2pipe.yaml");
        fout_n2p << device_descs_net2pipe;
        fout.close();
    }
}
...
void handle_pipegen2_compile_exception(
    const pipegen2::BasePipegen2CompileException &ex,
    const std::string &graph_name,
    const int temporal_epoch,
    const std::unordered_map<chip_id_t, buda_soc_description> &sdesc_per_chip,
    tt_compile_result_per_epoch &compile_result) {
    ...
    tt_cxy_pair logical_error_core = get_pipegen2_exception_logical_core(ex, sdesc_per_chip);
    compile_result.device_id = logical_error_core.chip;
...
tt_cxy_pair get_pipegen2_exception_logical_core(
    ...
    if (ex.get_logical_core_location().has_value()) {
        return ex.get_logical_core_location().value();
    } else {
        // It is guaranteed that the 'ex' will have either logical or physical core location set (it can also have both),
        // so it is safe to call .value() on physical core location optional if the logical core location optional is
        // empty.
        return pipegen2::convert_physical_core_to_logical(ex.get_physical_core_location().value(), sdesc_per_chip);
    }
}

void handle_pipegen2_io_exception(
...
void translate_workers_and_eth(tt_cluster* cluster, const chip_id_t target_device, const std::string& soc_descriptor_path, const std::string& output_dir) {
    std::unique_ptr<buda_SocDescriptor> translated_soc_descriptor = load_soc_descriptor_from_yaml(soc_descriptor_path);
        for(auto& worker : translated_soc_descriptor -> workers) {
            cluster -> translate_to_noc_table_coords(target_device, worker.y, worker.x);
        }
        for(auto& eth_core : translated_soc_descriptor -> ethernet_cores) {
            cluster -> translate_to_noc_table_coords(target_device, eth_core.y, eth_core.x);
        }
        ...
        desc_name << output_dir << "/device_descs/" << target_device << ".yaml";
    ...
    if (e.find("blob command failed") != std::string::npos) {
        ...
        if (e.find("The overlay blob") != std::string::npos) {
            ...
            if (std::regex_search(e, match, regex) && match.size() > 1) {
                ...
                result.allocated_size_bytes = size;
                result.maximum_size_bytes = l1_mem::address_map::OVERLAY_BLOB_SIZE;
...
void run_netlist_analyzer(tt::ARCH arch, const string &netlist_path, const string &cluster_desc_path, const string &test_output_dir) {
    bool run_en = parse_env("TT_BACKEND_NETLIST_ANALYZER", false);
    if (!run_en) return;

    const string arch_str = get_arch_str(arch);
    if (arch_str != "grayskull" and arch_str != "wormhole_b0") {
        log_warning(tt::LogAnalyzer, "Skipping netlist analyzer run because it does not support arch {}", arch_str);
        return;
    }
    log_info(tt::LogAnalyzer, "Running Netlist Analyzer");
    const string budabackend_root = buda_home();
    log_assert(budabackend_root != "", "Netlist Analyzer failed to read the budabackend root. BUDA_HOME env var is not set.");
    const string test_path = budabackend_root + "/build/bin/dpnra";
    const string output_path = test_output_dir;
    const string log_path = output_path + "/" + "netlist_analyzer.log";
    const string err_path = output_path + "/" + "netlist_analyzer.err";

    if (!fs::exists(output_path)) {
        fs::create_directory(output_path);
    }

    // run the netlist analyzer
    stringstream netlist_analyzer_cmd;
    netlist_analyzer_cmd << "ARCH_NAME=" << arch_str;
    netlist_analyzer_cmd << " " << test_path;
    netlist_analyzer_cmd << " " << "--arch" << " " << arch_str;
    netlist_analyzer_cmd << " " << "--netlist" << " " << netlist_path;
    netlist_analyzer_cmd << " " << "--out-dir" << " " << output_path;

    if (arch_str == "wormhole_b0") {
        netlist_analyzer_cmd << " " << "--cluster-desc" << " " << cluster_desc_path;
    }

    log_info(tt::LogAnalyzer, "Netlist analyzer command: {}", netlist_analyzer_cmd.str());
    auto result = tt::run_command(netlist_analyzer_cmd.str(), log_path, err_path);
    if (result.success) {
        log_info(tt::LogAnalyzer, "Netlist analyzer finished successfully");
        log_info(tt::LogAnalyzer, "Writing netlist analyzer results to: {}", output_path);
    }
    else {
        log_error("Error log: {}", result.message);
        log_fatal("Netlist analyzer failed. Full log under {}", err_path);
    }
}
